...
public class SuitVariantRecipeHandler implements StandardRecipeHandler<SuitVariantMenu> {
    ...
    public boolean craft(EmiRecipe recipe, EmiCraftContext<SuitVariantMenu> context) {
        ...
        List<SuitVariantRecipe> recipes = world.getRecipeManager().getRecipesFor(MarvelRecipeTypes.SUIT_VARIANT.get(), new SingleRecipeInput(recipe.getInputs().get(0).getEmiStacks().get(0).getItemStack()), world).stream().map(RecipeHolder::value).toList();
