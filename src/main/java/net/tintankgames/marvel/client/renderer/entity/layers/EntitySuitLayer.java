...
public class EntitySuitLayer<T extends LivingEntity, M extends HumanoidModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int light, T living, float f1, float f2, float f3, float f4, float f5, float f6) {
        ...
        if (model != null) {
            ...
            renderColoredCutoutModel(model, living.getItemBySlot(EquipmentSlot.HEAD).getOrDefault(MarvelDataComponents.HELMET_OPEN, false) ? living.getData(MarvelAttachmentTypes.ENTITY_SUIT).openTexture() : living.getData(MarvelAttachmentTypes.ENTITY_SUIT).texture(), poseStack, multiBufferSource, light, living, 1.0F, 1.0F, 1.0F);
            if (EnergySuitItem.getEnergy(living.getItemBySlot(EquipmentSlot.CHEST)) > 0.0F) GlowLayer.renderColoredEmissiveModel(model, living.getItemBySlot(EquipmentSlot.HEAD).getOrDefault(MarvelDataComponents.HELMET_OPEN, false) ? living.getData(MarvelAttachmentTypes.ENTITY_SUIT).openTexture().withPath(path -> path.replace(".png", "_glow.png")) : living.getData(MarvelAttachmentTypes.ENTITY_SUIT).texture().withPath(path -> path.replace(".png", "_glow.png")), poseStack, multiBufferSource, light, living, 1.0F, 1.0F, 1.0F);
        }
    }
