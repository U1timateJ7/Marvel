...
public class MarvelBlocks {
    ...
    public static final DeferredBlock<Block> DEEPSLATE_BRICK_LAMP = register("deepslate_brick_lamp", () -> new CopperBulbBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.STONE_BRICKS).lightLevel(state -> state.getValue(CopperBulbBlock.LIT) ? 15 : 9)));
    public static final DeferredBlock<Block> TESSERACT = registerBlockOnlyWithType("tesseract", () -> TesseractBlock.CODEC, () -> new TesseractBlock(BlockBehaviour.Properties.of().mapColor(MapColor.COLOR_LIGHT_BLUE).instrument(NoteBlockInstrument.HAT).sound(SoundType.GLASS).emissiveRendering((state, getter, pos) -> true).lightLevel(state -> 10).strength(0.5F).pushReaction(PushReaction.NORMAL).explosionResistance(1200.0F)));
