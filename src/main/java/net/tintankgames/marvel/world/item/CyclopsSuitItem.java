...
public class CyclopsSuitItem extends LeatherSuitItem {
    public CyclopsSuitItem(Type type, Properties properties) {
        super(type, true, MarvelItems.Tags.CYCLOPS_ARMOR, type == Type.CHESTPLATE ? List.of(effect(MobEffects.JUMP, 0), effect(MobEffects.MOVEMENT_SPEED, 0)) : List.of(), type == Type.HELMET ? properties.component(MarvelDataComponents.POWER_ITEMS, List.of(MarvelItems.OPTIC_BLAST.get())).component(MarvelDataComponents.OPTIC_BLAST_MODE, OpticBlastMode.NARROW) : properties.component(MarvelDataComponents.POWER_ITEMS, List.of(MarvelItems.OPTIC_BLAST.get())));
    }
