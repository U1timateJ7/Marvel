        ...
        for (int i = 0; i < p_44497_.getContainerSize(); i++) {
            ...
            if (!stack1.isEmpty()) {
                if (stack1.is(MarvelItems.IRON_MAN_MARK_38_HELMET_COMPONENT) && helmet.isEmpty()) {
                    helmet = stack1.transmuteCopy(BuiltInRegistries.ITEM.get(new ResourceLocation(stack1.getItemHolder().getRegisteredName().replace("_component", ""))), stack1.getCount());
                } else if (stack1.is(MarvelItems.IRON_MAN_MARK_38_CHESTPLATE_COMPONENT) && chestplate.isEmpty()) {
                    chestplate = stack1.transmuteCopy(BuiltInRegistries.ITEM.get(new ResourceLocation(stack1.getItemHolder().getRegisteredName().replace("_component", ""))), stack1.getCount());
                } else if (stack1.is(MarvelItems.IRON_MAN_MARK_38_LEGGINGS_COMPONENT) && leggings.isEmpty()) {
                    leggings = stack1.transmuteCopy(BuiltInRegistries.ITEM.get(new ResourceLocation(stack1.getItemHolder().getRegisteredName().replace("_component", ""))), stack1.getCount());
                } else if (stack1.is(MarvelItems.IRON_MAN_MARK_38_BOOTS_COMPONENT) && boots.isEmpty()) {
                    boots = stack1.transmuteCopy(BuiltInRegistries.ITEM.get(new ResourceLocation(stack1.getItemHolder().getRegisteredName().replace("_component", ""))), stack1.getCount());
                }
            }
        }
