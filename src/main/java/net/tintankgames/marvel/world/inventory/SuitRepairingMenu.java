...
public class SuitRepairingMenu extends AbstractContainerMenu implements SuitTableMenu {
    ...
    protected SuitRepairingMenu(int p_38852_, Inventory p_39357_) {
        this(p_38852_, p_39357_, ContainerLevelAccess.NULL, new SimpleContainerData(4));
    }

    public SuitRepairingMenu(int p_38852_, Inventory inventory, ContainerLevelAccess containerLevelAccess, ContainerData data) {
        super(MarvelMenuTypes.SUIT_REPAIRING.get(), p_38852_);
        this.access = containerLevelAccess;
        this.player = inventory.player;
        checkContainerDataCount(data, 4);
        this.data = data;
        this.addSlot(new SuitRepairingResultSlot(inventory.player, this.craftSlots, this.resultSlot, 0, 143, 33));

        this.addSlot(new Slot(this.craftSlots, 0, 20, 33));

        this.addSlot(new Slot(this.craftSlots, 1, 68, 24));
        this.addSlot(new Slot(this.craftSlots, 2, 86, 24));
        this.addSlot(new Slot(this.craftSlots, 3, 68, 42));
        this.addSlot(new Slot(this.craftSlots, 4, 86, 42));
    ...
    protected static void slotChangedCraftingGrid(AbstractContainerMenu p_150547_, Level p_150548_, Player p_150549_, CraftingContainer p_150550_, ResultContainer p_150551_, @Nullable RecipeHolder<SuitRepairingRecipe> p_345124_) {
        if (!p_150548_.isClientSide) {
            ServerPlayer serverplayer = (ServerPlayer)p_150549_;
            ItemStack itemstack = ItemStack.EMPTY;
            Optional<RecipeHolder<SuitRepairingRecipe>> optional = p_150548_.getServer().getRecipeManager().getRecipeFor(MarvelRecipeTypes.SUIT_REPAIRING.get(), p_150550_.asCraftInput(), p_150548_, p_345124_);
            if (optional.isPresent()) {
                RecipeHolder<SuitRepairingRecipe> recipeholder = optional.get();
                SuitRepairingRecipe upgradingRecipe = recipeholder.value();
                if (p_150551_.setRecipeUsed(p_150548_, serverplayer, recipeholder)) {
                    ItemStack itemstack1 = upgradingRecipe.assemble(p_150550_.asCraftInput(), p_150548_.registryAccess());
    ...
    public ItemStack quickMoveStack(Player p_39391_, int p_39392_) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (p_39392_ == 0) {
            ...
            } else if (p_39391_.level().getRecipeManager().getAllRecipesFor(MarvelRecipeTypes.SUIT_REPAIRING.get()).stream().anyMatch(recipe -> recipe.value().getSuit().test(itemstack1))) {
                if (!this.moveItemStackTo(itemstack1, 1, 2, false)) {
                    return ItemStack.EMPTY;
                }
            } else if (p_39392_ >= 6 && p_39392_ < 42) {
                ...
                else if (!this.moveItemStackTo(itemstack1, 2, 6, false)) {
                    if (p_39392_ < 33) {
                        if (!this.moveItemStackTo(itemstack1, 33, 42, false)) {
                            return ItemStack.EMPTY;
                        }
