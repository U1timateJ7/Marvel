...
public class MjolnirEntity extends AbstractArrow implements ItemSupplier {
	public MjolnirEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(MarvelModEntityTypes.MJOLNIR.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(MarvelModItems.MJOLNIR.get());
	}
	...
	public static MjolnirEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		MjolnirEntity entityarrow = new MjolnirEntity(MarvelModEntityTypes.MJOLNIR.get(), entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(),
				MarvelModSounds.get(new ResourceLocation("marvel:item.mjolnir.throw")), SoundSource.PLAYERS, 1,
				1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static MjolnirEntity shoot(LivingEntity entity, LivingEntity target) {
		MjolnirEntity entityarrow = new MjolnirEntity(MarvelModEntityTypes.MJOLNIR.get(), entity, entity.level);
