...
public class RepulsorEntity extends AbstractArrow implements ItemSupplier {
	public RepulsorEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		super(MarvelModEntities.REPULSOR, world);
	}
	...
	public static RepulsorEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		RepulsorEntity entityarrow = new RepulsorEntity(MarvelModEntities.REPULSOR, entity, world);
		...
		world.playSound((Player) null, entity.getX(), entity.getY(), entity.getZ(),
				ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("marvel:item.repulsor.shoot")), SoundSource.PLAYERS, 1,
				1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static RepulsorEntity shoot(LivingEntity entity, LivingEntity target) {
		RepulsorEntity entityarrow = new RepulsorEntity(MarvelModEntities.REPULSOR, entity, entity.level);
