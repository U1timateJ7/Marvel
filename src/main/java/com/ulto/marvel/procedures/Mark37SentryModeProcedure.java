...
public class Mark37SentryModeProcedure {
		...
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands()
					.performCommand(
							...
							("item replace entity " + sentryUUID + " armor.head with marvel:iron_man_mark_37_helmet"));
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands()
					.performCommand(
							new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", new TextComponent(""),
									_level.getServer(), null).withSuppressedOutput(),
							("item replace entity " + sentryUUID + " armor.chest with marvel:iron_man_mark_37_chestplate"));
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands()
					.performCommand(
							new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", new TextComponent(""),
									_level.getServer(), null).withSuppressedOutput(),
							("item replace entity " + sentryUUID + " armor.legs with marvel:iron_man_mark_37_leggings"));
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands()
					.performCommand(
							new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", new TextComponent(""),
									_level.getServer(), null).withSuppressedOutput(),
							("item replace entity " + sentryUUID + " armor.feet with marvel:iron_man_mark_37_boots"));
