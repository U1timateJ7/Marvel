...
public class IronSpiderSuitUpProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (entity instanceof Player player) {
			ItemStack storage = player.getInventory().armor.get(2);
			ItemStack helmet = ItemStack.of(storage.getOrCreateTag().getList("ArmorItems", 10).getCompound(3));
			ItemStack chestplate = ItemStack.of(storage.getOrCreateTag().getList("ArmorItems", 10).getCompound(2));
			ItemStack leggings = ItemStack.of(storage.getOrCreateTag().getList("ArmorItems", 10).getCompound(1));
			ItemStack boots = ItemStack.of(storage.getOrCreateTag().getList("ArmorItems", 10).getCompound(0));
			if (IronManSuitItem.getBattery(helmet) > 0 && IronManSuitItem.getBattery(chestplate) > 0 && IronManSuitItem.getBattery(leggings) > 0 && IronManSuitItem.getBattery(boots) > 0) {
				...
				if (player.level.isClientSide()) player.displayClientMessage(new TranslatableComponent("item.marvel.iron_spider_storage.out_of_power"), false);
			}
		}
	}
}
